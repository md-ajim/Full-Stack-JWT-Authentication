...
export function SignUpForm(props) {
  ...
  const [otpValueChange, setOtpValueChange] = useState("");
  const [otpIsValid, setOtpIsValid] = useState(false);
  const [disabledInput, setDisabledInput] = useState(false);
  const [submitDisabled, setSubmitDisabled] = useState(false);

  const handleOtpInputValidCheck = (OTP) => {
    ...
    if (sentOtp) {
      ...
      try {
        ...
        if (response.status === 201) {
          ...
          if (res.ok) {
            toast.success(
              `The register is successfully Welcome to the dashboard`
            );
            window.location.href = "/dashboard";
          }
          if (res.error) {
            toast.error(`This input is wrong. Please give valid input.`);
            console.error(`Error: ${res.error}`);
          }
        }
      } catch (err) {
        toast.error(`Please check is otp code`);
      ...
      try {
        ...
        if (response.status === 200) {
          toast.success(`Send OTP code successfully. Please Check Your Email.`);
          setSentOtp(true);
          setDisabledInput(true);
        ...
        toast.error(`OTP code send field`);
        console.error(`send otp is field${error}`);
      } finally {
        setLoading(false);
        toast.dismiss(loadingId);
      }
            ...
            <div className="flex flex-col gap-6">
              ...
              {sentOtp ? (
                <div className="grid gap-1">
                  ...
                  <InputOTP
                    maxLength={6}
                    onChange={(OTP) => handleOtpInputValidCheck(OTP)}
                  >
                    <InputOTPGroup>
                    ...
                    {otpExpired ? (
                      ...
                      <Button disabled variant={otpIsValid ? "outline" : ""}>
                        {" "}
                        {otpIsValid ? "Valid" : "Waiting..."}
                      </Button>
                    )}
                  </InputOTP>
              ...
              <div className="grid-cols-3 place-items-center  gap-4">
